...
public class MovementSpeedAttributeModifierReward extends AbstractReward {
    ...
    /**
     * 구버전 호환/참고용 UUID (현재 26.2에서는 AttributeModifier 식별자로 사용하지 않음)
     */
    public static UUID computeModifierUuid(Identifier holderId, Identifier actionId) {
        String key = "arc:movement_speed_modifier:" + holderId + ":" + actionId;
        return UUID.nameUUIDFromBytes(key.getBytes(StandardCharsets.UTF_8));
    }

    /**
     * 26.2 기준 AttributeModifier 식별자는 Identifier입니다.
     * 신규 코드는 이 id를 사용합니다.
     */
    public static Identifier computeModifierId(Identifier holderId, Identifier actionId) {
        ...
        // 26.2에서는 생성자 대신 팩토리 메서드 사용
        return Identifier.fromNamespaceAndPath("arc", path);
    }
    ...
    /**
     * 구버전 호환/디버그용 name (현재 26.2에서는 AttributeModifier에 name이 없음)
     */
    public static String computeModifierName(Identifier holderId, Identifier actionId) {
