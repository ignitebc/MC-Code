...
실제 사용 중인 M416 외형을 기본 리소스로 연결했고, 사용되지 않던 이전 모델 변형은 이후 미사용 자산 정리에서 제거했습니다. DB-4가 공유하던 소드오프 사운드 경로도 함께 갱신했습니다. 외부 총기팩·명령·설정에서 직접 리소스 경로를 지정한 경우에는 새 경로로 갱신해야 합니다. 호환 매핑은 기존 총기·display ID를 위한 것이며, 모든 외부 파일 경로의 별칭을 제공하지는 않습니다.
