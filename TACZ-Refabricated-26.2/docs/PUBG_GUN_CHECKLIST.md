...
이번 반영은 요청한 5종의 이름·내부 식별자 통일과 SLR 기본 총기팩 추가입니다.

## 이름·내부 식별자 변경 — 5종

| 반영 | 표시 이름 | 내부 ID | 번역 키 |
|---|---|---|---|
| ☑ | M416 | `tacz:m416` | `tacz.gun.m416.name` |
| ☑ | P18C | `tacz:p18c` | `tacz.gun.p18c.name` |
| ☑ | 소드오프 / Sawed-Off | `tacz:sawed_off` | `tacz.gun.sawed_off.name` |
| ☑ | MP5K | `tacz:mp5k` | `tacz.gun.mp5k.name` |
| ☑ | Micro UZI | `tacz:micro_uzi` | `tacz.gun.micro_uzi.name` |

등록·설정·제작법·부착물 태그·번역 키와 파일명, 모델 뼈대·애니메이션·스크립트·사운드·텍스처 참조, 크리에이티브 탭·제작대 아이콘 및 도감 문서를 같은 이름으로 맞췄습니다. 총기별 함수·변수와 Lua 스크립트도 검색했으며, 확인된 전용 식별자를 함께 변경했습니다. 공통 발사 로직의 범용 함수명은 유지합니다.

이전 총기 ID와 기본 display ID는 `GunIdAliases`의 5종 호환 매핑으로 해석합니다. 아이템의 ID를 읽는 동작은 NBT를 수정하지 않으며, ID를 설정할 때 새 이름으로 저장합니다. 탄약·부착물·경험치 등 다른 저장 필드는 변경하지 않습니다. API의 총기 조회·display 조회에도 같은 매핑을 적용하며, 이전 이름의 별도 총기는 등록하지 않습니다. 커스텀 네임스페이스와 다른 총기의 ID는 그대로 전달합니다.

실제 사용 중인 M416 외형을 기본 리소스로 연결했고, 사용되지 않던 이전 모델 변형은 `m416_legacy` 파일명으로 보존했습니다. DB-4가 공유하던 소드오프 사운드 경로도 함께 갱신했습니다. 외부 총기팩·명령·설정에서 직접 리소스 경로를 지정한 경우에는 새 경로로 갱신해야 합니다. 호환 매핑은 기존 총기·display ID를 위한 것이며, 모든 외부 파일 경로의 별칭을 제공하지는 않습니다.
...
모듈의 README와 이 문서도 갱신합니다. Java의 총기·display ID 읽기/설정 및 조회 경로에 호환 처리를 추가했습니다. 기존 총기의 전투 설정과 외형 데이터는 유지합니다.
...
- [x] 5종의 내부 ID·번역 키·파일명·리소스 참조 변경 및 이전 이름 잔여 검색
- [x] 기존 총기 설정·외형·애니메이션 수치와 바이너리 자산 내용 보존 확인
- [x] 이전 총기·display ID의 읽기/설정 및 API 조회 호환 경로 정적 확인
