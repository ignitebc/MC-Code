...
public class SpecialItemReward extends AbstractReward
{
    ...
    private final ResourceLocation bonusPowerupLine;
    ...
    public SpecialItemReward(double chance, int priority, ItemStack itemStack, int amount,
                             ResourceLocation bonusPowerupLine, double bonusChancePerTier)
    {
    ...
    private static int getHighestActiveTier(JobsPlayer jobsPlayer, ResourceLocation powerupLine)
    {
        for (int index = TIER_SUFFIXES.length - 1; index >= 0; index--)
        {
            ResourceLocation powerupLocation = ResourceLocation.fromNamespaceAndPath(
    ...
    private static boolean isPowerupActive(JobsPlayer jobsPlayer, ResourceLocation powerupLocation)
    {
    ...
    public static class Serializer implements IRewardSerializer<SpecialItemReward>
    {
        ...
        public SpecialItemReward fromJson(JsonObject jsonObject, double chance, int priority)
        {
            ...
            ResourceLocation bonusPowerupLine = null;
            if (jsonObject.has("bonus_powerup"))
            {
                bonusPowerupLine = ResourceLocation.parse(GsonHelper.getAsString(jsonObject, "bonus_powerup"));
            }
