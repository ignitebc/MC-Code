...
public class ConditionsComponent extends EmptyComponent
{
    ...
    public ConditionsComponent(List<ICondition> conditions, Supplier<ScreenRectangle> parentBounds)
    {
        ...
        {
            TruncatedTextComponent noConditionsText = new TruncatedTextComponent(
                    0,
                    2,
                    getWidth(),
        ...
        {
            TruncatedTextComponent title = new TruncatedTextComponent(
                    0,
                    2,
                    getWidth(),
                    JobsPlus.translatable("gui.jobs.conditions").withStyle(style -> style.withUnderlined(true)),
                    0xFF1E1410
            );
    ...
    private static Component getKoreanConditionDescription(String original)
    {
        ...
        // experience level is … 부분 – 예외 방어 추가
        int idx = lower.indexOf("experience level is");
        if (idx >= 0)
        {
            String after = original.substring(idx + "experience level is".length()).trim();

            // 빈 문자열이면 substring(0, -1) 방지
            if (after.isEmpty())
            {
                return Component.literal("인챈트 레벨 조건");
            }

            // 끝이 '.' 이고, 길이가 1 초과일 때만 잘라냄
            if (after.endsWith(".") && after.length() > 1)
            {
