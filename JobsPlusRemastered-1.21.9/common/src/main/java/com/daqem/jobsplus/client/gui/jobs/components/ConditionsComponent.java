...
public class ConditionsComponent extends EmptyComponent
{
        ...
        else
        {
            ...
            for (ICondition condition : conditions)
            {
                ...
                if (condition instanceof BlockCondition || condition instanceof BlocksCondition)
                {
                    ...
                    List<ICondition> notBlockConditions = notConditions.stream().map(NotCondition::getConditions).flatMap(List::stream).filter(c -> c instanceof BlockCondition || c instanceof BlocksCondition || c instanceof BlockHardnessCondition || c instanceof IsOreCondition).toList();
                ...
                if (condition instanceof IsOreCondition isOreCondition)
                {
                    ...
                    if (conditions.stream().noneMatch(b -> b instanceof BlockCondition || b instanceof BlocksCondition))
                    {
                        List<ICondition> notBlockConditions = notConditions.stream().map(NotCondition::getConditions).flatMap(List::stream).filter(c -> c instanceof BlockCondition || c instanceof BlocksCondition || c instanceof BlockHardnessCondition).toList();
                ...
                if (condition instanceof ItemCondition || condition instanceof ItemsCondition)
                {
                    ...
                    List<ICondition> notItemConditions = notConditions.stream().map(NotCondition::getConditions).flatMap(List::stream).filter(c -> c instanceof ItemCondition || c instanceof ItemsCondition).toList();
    ...
    private List<Block> getOreBlocks()
    {
        if (ORE_BLOCKS == null)
        {
            ...
            if (level == null)
                return List.of();
            List<Block> oreBlocks = level.registryAccess().lookupOrThrow(Registries.BLOCK).stream().filter(block -> new IsOreCondition(false).isMet(new ActionDataBuilder((ArcPlayer) Minecraft.getInstance().player, ActionType.BREAK_BLOCK).withData(ActionDataType.BLOCK_STATE, block.defaultBlockState()).build())).toList();
