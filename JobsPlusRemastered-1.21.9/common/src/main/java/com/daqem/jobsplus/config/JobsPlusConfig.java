...
public class JobsPlusConfig
{
    ...
    static
    {
        ...
        // 직업갯수 2개로 조정 (원본)
        // maxJobs = config.defineInteger("max_jobs", Integer.MAX_VALUE, 0,
        // Integer.MAX_VALUE).withComments("the maximum amount of jobs a player can
        // have");
        maxJobs = config.defineInteger("max_jobs", 2, 0, Integer.MAX_VALUE).withComments("플레이어가 가질 수 있는 최대 직업 수");
        config.push("coins");

        // coinsPerLevelUp = config.defineInteger("coins_per_level_up", 1, 0,
        // Integer.MAX_VALUE).withComments("the amount of coins a player gets when they
        // level up a job");
        // 레벨업당 코인 얻는양 3개로 조정
