...
public class PowerupManager extends SimplePreparableReloadListener<List<IActionHolder>>
{
    ...
    {
        Map<Identifier, Resource> resourceMap = resourceManager.listResources("jobsplus/powerups", (resourceLocation) -> resourceLocation.getPath().endsWith(".json")).entrySet().stream().collect(Collectors.toMap(entry -> Identifier.fromNamespaceAndPath(entry.getKey().getNamespace(), entry.getKey().getPath().substring(0, entry.getKey().getPath().length() - ".json".length()).substring("jobsplus/powerups/".length())), Map.Entry::getValue));
