...
public class JobManager extends SimplePreparableReloadListener<List<IActionHolder>>
{
    ...
    {
        Map<Identifier, Resource> resourceMap = resourceManager.listResources("jobsplus/jobs", (resourceLocation) -> resourceLocation.getPath().endsWith(".json")).entrySet().stream().collect(Collectors.toMap(entry -> Identifier.fromNamespaceAndPath(entry.getKey().getNamespace(), entry.getKey().getPath().substring(0, entry.getKey().getPath().length() - ".json".length()).substring("jobsplus/jobs/".length())), Map.Entry::getValue));
