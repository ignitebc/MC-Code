...
public class UserGuideScrollComponent extends EmptyComponent
{
    ...
    private static final String USER_GUIDE_LEFT = """
            ...
            - 삭제된 아이템은 되찾을 수 없으며, 무엇이 사라졌는지는 서버 전체에 공지됩니다. (예: OOO님이 죽어, 금 도끼, 금 투구이(가) 사라집니다.)</red>
    ...
    private static final String USER_GUIDE_RIGHT = """
            ...
            - 등급별 펫 종류와 보유 수는 펫관리 탭의 펫도감에서 볼 수 있습니다.
            ...
            - 펫의 1레벨 능력치는 등급에 따라 다릅니다. 일반: 공격력 +1·체력 100 / 희귀: 공격력 +2·체력 300 / 전설: 공격력 +3·체력 500
            - 펫은 1레벨로 시작하며, 몹을 한 대 때릴 때마다 경험치 1을 얻습니다. 필요 경험치는 직업 레벨과 같습니다.
            - 레벨이 오를 때마다 공격력 +0.1, 체력 +10이 됩니다. 이름 옆에 레벨이 표시되고 이름 색은 등급을 나타냅니다.
            - 펫은 몹이나 다른 플레이어에게 맞으면 체력이 줄고, 싸우지 않는 동안 조금씩 회복합니다. 낙하·화염 같은 환경 피해는 받지 않으며 공허에 떨어져도 주인 곁으로 돌아옵니다.
            - 펫이 죽으면 (레벨 × 10)초 뒤에 부활합니다. 부활한 펫은 OFF 상태이므로 펫관리 탭에서 다시 켜야 합니다.
            - 펫에게는 먹이를 줄 수 없고 번식도 되지 않으며, 주인을 밀치지 않습니다.
            - 주인이 접속을 종료하면 펫도 함께 사라지고, 다시 접속하면 곁에 다시 소환됩니다.
            - 펫관리 탭의 ON/OFF 단추로 펫을 넣고 꺼낼 수 있습니다. OFF 상태의 펫은 보이지 않습니다.
            - 펫관리 탭에서 펫 이름을 바꿀 수 있습니다. 이름과 레벨은 펫 머리 위에 작게 표시됩니다.
            - 주인이 죽으면 펫이 모두 회수되고 OFF가 됩니다. 부활한 뒤 펫관리 탭에서 다시 켜야 합니다.
            ...
            ■ 장비 등급 (S~F)
            - 내구도가 있는 모든 장비에는 S·A·B·C·D·E·F 일곱 등급 중 하나가 같은 확률(각 약 14.3%)로 붙습니다.
            - 등급은 제작·상자·상점·랜덤 상자 등 어떤 경로로 얻든 인벤토리에 들어온 순간 정해지며, 한 번 정해지면 바뀌지 않습니다.
            - 등급은 아이템 이름 바로 아래에 빨간 글씨로 표시됩니다. (예: B티어)
            - S는 원래 수치 그대로이고, 한 등급 내려갈 때마다 내구도는 7%, 채굴 속도·공격력·방어도는 4%씩 낮아집니다. F는 내구도 58%, 나머지 76%입니다.
            ...
            - 총을 든 몬스터는 탄약 없이 사격합니다. 멀리서도 맞을 수 있으니 주의하세요.</red>
