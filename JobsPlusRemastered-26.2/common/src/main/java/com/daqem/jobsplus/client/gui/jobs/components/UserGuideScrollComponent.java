...
public class UserGuideScrollComponent extends EmptyComponent
{
    ...
    private static final String USER_GUIDE = """
            ...
            ■ 직업 시작하기
            - J 키를 누르면 직업 화면이 열리며, 좌측 [직업] 탭에서 원하는 직업을 선택해 시작할 수 있습니다.
            - 처음에는 무료로 직업 2개를 가질 수 있으며, 직업 선택권을 사용하면 최대 8개까지 늘릴 수 있습니다.
            - 직업 레벨이 오를 때마다 스킬 해방에 사용하는 직업 코인 20개가 지급됩니다.
            ...
            ■ 사망 시 아이템 손실과 보존권
            <red>- 사망하면 소지품이 사망 지점 바닥에 떨어지며, 그중 무작위로 한 칸은 바닥에 떨어지지 않고 완전히 삭제됩니다.
            - 삭제된 칸의 아이템은 다시 주울 수 없으니 주의하세요.</red>
            - 사망 시 아이템 보존권을 소지하고 있으면(방어구·보조 손 포함) 자동으로 1개가 소모되어 모든 소지품을 지킵니다.
            - 보존권이 발동하면 아이템이 바닥에 떨어지지 않고 삭제도 없이 그대로 유지된 채 리스폰합니다.
            ...
            ■ 상점 판매 품목 (에메랄드 획득)
            - 아래 수량을 지불하면 에메랄드 1개를 받습니다. 
            - 주민 거래 대비 -50% 효율이므로 주민 거래 효율이 훨씬 좋습니다.
        
            ■ 청크 구매와 보호
            - 청크 구매 지도서를 손에 들고 우클릭하면 서 있는 청크 1칸(16X16)이 본인 소유가 되고 지도서 1개가 소모됩니다.
            - 이미 주인이 있는 청크에서는 지도서가 소모되지 않고 안내 메시지만 표시됩니다.
            ...
            - 보상은 인벤토리로 바로 지급되며, 인벤토리가 가득 차면 남은 보상만 주변 바닥에 떨어집니다.
            - 인벤토리가 가득 찬 상태라면 보상이 용암이나 낭떠러지에 떨어지지 않도록 안전한 장소에서 사용해 주세요.
            ...
            일반 펫 상자: 디아루가 펫 (50%), 커비 펫 (50%)
            희귀 펫 상자: 유니콘 펫 (50%), 가젤 펫 (50%)
            전설 펫 상자: 페어린 펫 (33.3%), 암흑드래곤 펫 (33.3%), 스컬큰 레이븐 펫 (33.3%)

            ■ 펫 설명
            - 소환된 펫은 즉시 길들여진 상태로 주인을 따라다니며, 주인과 멀어지면 곁으로 순간이동합니다.
            - 주인이 다른 차원으로 이동하면 펫도 함께 따라옵니다.
            - 주인을 공격한 대상은 그 대상이 죽을 때까지 끝까지 추격해 공격합니다.
            - 펫의 공격력은 상자 등급에 따라 다릅니다. 일반 +1, 희귀 +3, 전설 +5
            - 펫은 어떤 공격에도 죽지 않으며, 공허에 떨어져도 주인 곁으로 돌아옵니다.
            - 펫에게는 먹이를 줄 수 없고 번식도 되지 않습니다.

            ■ 네더라이트 장비 업그레이드 방법
            - 네더라이트 장비는 잿빛 → 태양 → 생명 → 서리빛 순서로 한 단계씩 업그레이드할 수 있습니다.
            - 업그레이드 주괴는 조합창에서 만듭니다. 잿빛 주괴 = 네더라이트 주괴 1개 + 철 주괴 4개
            - 태양 주괴 = 잿빛 주괴 1개 + 금 주괴 4개 / 생명 주괴 = 태양 주괴 1개 + 에메랄드 4개
            - 서리빛 주괴 = 생명 주괴 1개 + 다이아몬드 4개
            - 대장장이 작업대에 [네더라이트 강화 형판 + 이전 단계 장비 + 다음 단계 주괴]를 넣으면 장비가 업그레이드됩니다.
            - 예시: 네더라이트 검 + 잿빛 주괴 → 잿빛 검, 잿빛 검 + 태양 주괴 → 태양 검
            - 네더라이트 창도 같은 방식으로 잿빛 → 태양 → 생명 → 서리빛 순서로 업그레이드할 수 있습니다.

            ■ 네더라이트 업그레이드 방어구 특수 효과
            - 잿빛 방어구: 팬텀이 먼저 공격하지 않습니다.
            - 태양 방어구: 피글린이 먼저 공격하지 않습니다.
            - 생명 방어구: 엔더맨과 눈이 마주쳐도 적대하지 않습니다.
            ...
            - 강화 제작대에서는 서리빛 네더라이트 장비를 최대 +10강까지 강화할 수 있습니다.
            ...
            - 검은 강화 단계마다 공격력 1, 도구는 채굴 효율 0.1, 방어구는 최대 체력 1(하트 반 칸)이 증가합니다.
            ...
            ■ 크리퍼 폭발 주의
            <red>- 이 서버는 크리퍼 폭발이 생명체에게 주는 피해가 일반 서버보다 강하게 조정되어 있습니다. (어려움 기준 약 2배)
            - 방어구 없이 근접 폭발을 맞으면 즉사할 수 있으니 주의하세요.</red>
            - 폭발의 지형 파괴 범위는 일반 서버와 같습니다.
